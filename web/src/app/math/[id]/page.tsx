...
export default async function MathCardDetailPage({ params }: MathCardDetailProps) {
  ...
  const mathKind = getMathKindFromCard(card) || card.reasoningType || "未分类";
  ...
  const blocks = [
    ...
    { title: detailLabels.extraNotesText || "补充备注", content: card.extraNotesText },
  ];

  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-5 py-8 sm:px-8 sm:py-10">
      <div className="flex items-center gap-3 text-sm text-[color:var(--muted)]">
        <Link href="/math" className="transition hover:text-[color:var(--accent-strong)]">
          数学题库
        </Link>
        <span>/</span>
        ...
        <div className="paper-card rounded-[38px] p-7 sm:p-10">
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="rounded-[24px] border border-[color:var(--line)] bg-white/70 p-5">
              <div className="text-xs uppercase tracking-[0.24em] text-[color:var(--muted)]">数学模块</div>
              <div className="mt-3 space-y-2 text-sm leading-7">
                <p>题型：{mathKind}</p>
                <p>模块：{mathModule || "未标注"}</p>
                <p>题目文本：{card.promptText ? "有" : "主要依赖附件"}</p>
              </div>
            </div>
            <div className="rounded-[24px] border border-[color:var(--line)] bg-white/70 p-5">
